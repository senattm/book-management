...
const createAuthor = async (req, res, next) => {
  try {
    ...
    return res
      .status(201)
      .json(new ApiResponse(data, "Yazar başarıyla oluşturuldu"));
  } catch (err) {
    next(err);
  }
};
...
const updateAuthor = async (req, res, next) => {
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(data, "Yazar başarıyla güncellendi"));
  } catch (err) {
    next(err);
  }
};
...
const deleteAuthor = async (req, res, next) => {
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(null, "Yazar başarıyla silindi"));
  } catch (err) {
    next(err);
  }
};
